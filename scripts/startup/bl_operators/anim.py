...
class NLA_OT_bake(Operator):
    ...
    def execute(self, context):
        ...
        if self.range_type == 'KEYFRAMES':
            if not hasattr(self, 'keyframes_range'):
                self.keyframes_range = anim_utils._keyframes_frame_range(context.selected_objects)

            if self.keyframes_range:
                self.frame_start, self.frame_end = self.keyframes_range
            else:
                self.frame_start, self.frame_end = anim_utils._scene_frame_range(context.scene)

        elif self.range_type == 'SCENE':
            if hasattr(self, 'scene_range'):
                self.frame_start, self.frame_end = self.scene_range
            else:
                self.frame_start, self.frame_end = anim_utils._scene_frame_range(context.scene)
    ...
    def invoke(self, context, _event):
        ...
        self.keyframes_range = anim_utils._keyframes_frame_range(context.selected_objects)
        self.scene_range = anim_utils._scene_frame_range(context.scene)

        if self.keyframes_range:
            self.frame_start = max(self.keyframes_range[0], self.scene_range[0])
            self.frame_end = min(self.keyframes_range[1], self.scene_range[1])
        else:
            self.frame_start, self.frame_end = self.scene_range
