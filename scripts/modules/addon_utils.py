...
def _initialize_once():
    ...
    for addon in _preferences.addons:
        enable(
            ...
            extensions_refresh_handled=True,
        )
...
def enable(module_name, *, default_set=False, persistent=False, extensions_refresh_handled=False, handle_error=None):
    """
    ...
    :arg extensions_refresh_handled: When true, :func:`extensions_refresh` must have been called with ``module_name``
    ...
    :type extensions_refresh_handled: bool
    ...
    if (is_extension := module_name.startswith(_ext_base_pkg_idname_with_dot)):
        ...
        # to ensure wheels setup in anticipation for this extension being used are removed.
        if not extensions_refresh_handled:
    ...
    if (
        ...
        if getattr(mod, "__addon_enabled__", False):
            ...
            try:
                ...
                handle_error(ex)
                if is_extension and not extensions_refresh_handled:
                    extensions_refresh()
                return None
...
def disable(module_name, *, default_set=False, extensions_refresh_handled=False, handle_error=None):
    """
    ...
    for path, pkg_id in _paths_with_extension_repos():
        ...
        for mod_name, _mod_path in _bpy.path.module_names(path, package=pkg_id):
            ...
            if is_enabled == is_loaded:
                ...
                enable(mod_name, extensions_refresh_handled=True)
            elif is_loaded:
...
def disable_all():
    ...
    for mod_name, mod in addon_modules:
        if getattr(mod, "__addon_enabled__", False):
            disable(mod_name, extensions_refresh_handled=True)
...
def _initialize_extension_repos_post_addons_prepare(
    ...
    for module_id_prev, module_id_next in submodules_rename_module:
        ...
        for submodule_id, mod in repo_runtime.items():
            ...
            disable(module_name_prev, default_set=False, extensions_refresh_handled=True)
    ...
    if submodules_del:
        ...
        for module_id in submodules_del:
            ...
            for submodule_id, mod in repo_runtime.items():
                ...
                disable(module_name_prev, default_set=default_set, extensions_refresh_handled=True)
            del repo
        del repo_module_map
    ...
    if submodules_del_disabled:
        for module_id_prev in submodules_del_disabled:
            ...
            for submodule_id in repo_userdef.keys():
                module_name_prev = "{:s}.{:s}.{:s}".format(_ext_base_pkg_idname, module_id_prev, submodule_id)
                disable(module_name_prev, default_set=True, extensions_refresh_handled=True)
...
def _initialize_extension_repos_post_addons_restore(addons_to_enable):
    extensions_refresh(addon_modules_pending=[module_name for (module_name, _, _, _) in addons_to_enable])
    if not addons_to_enable:
        return

    any_failled = False
    for (module_name, addon, default_set, persistent) in addons_to_enable:
        # Ensure the preferences are kept.
        if addon is not None:
            addon.module = module_name
        if enable(module_name, default_set=default_set, persistent=persistent, extensions_refresh_handled=True) is None:
            any_failled = True
