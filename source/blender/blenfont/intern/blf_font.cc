                                     ...
                                     void *user_data)
{
  ...
  if (U.experimental.use_text_shaping) {
    GlyphCacheBLF *gc = blf_glyph_cache_acquire(font);
    blf_shaping_foreach(font, gc, str, strlen(str), user_fn, user_data);
    blf_glyph_cache_release(font);
    return;
  }
#endif
                                           ...
                                           int location_x)
{
  ...
  else if (BLI_char_isRTL_utf8(&str[data.r_offset])) {
    /* clicking between RTL characters. */
    int offset = int(data.r_offset);
    BLI_str_cursor_step_next_utf8(str, int(str_len), &offset);
