      ...
      /* Debug only, helps spotting mismatches between menu entries and shortcuts! */
        ...
        if (is_strict && kmi->ptr) {
          ...
          if (ot) {
            ...
            PointerRNA opptr = RNA_pointer_create_isolated(nullptr, ot->srna, properties_default);
                                         ...
                                         wmKeyMap **r_keymap)
{
  ...
  if (!found && properties) {
    if (ot && ot->prop) { /* XXX Shall we also check ot->prop is actually an enum? */
      ...
      PointerRNA opptr = RNA_pointer_create_isolated(nullptr, ot->srna, properties_temp);
