...
static PointerRNA rna_NodeTreeInterfaceItem_parent_get(PointerRNA *ptr)
{
  ...
  PointerRNA result = RNA_pointer_create_isolated(&ntree->id, &RNA_NodeTreeInterfacePanel, parent);
  return result;
}
                                                       ...
                                                       StructFreeFunc free)
{
  ...
  PointerRNA dummy_socket_ptr = RNA_pointer_create_isolated(
...
static PointerRNA rna_NodeTreeInterfaceItems_active_get(PointerRNA *ptr)
{
  ...
  PointerRNA r_ptr = RNA_pointer_create_isolated(
...
static bool rna_NodeTreeInterface_items_lookup_int(PointerRNA *ptr, int index, PointerRNA *r_ptr)
{
  ...
  *r_ptr = RNA_pointer_create_with_ancestors(
      *ptr, &RNA_NodeTreeInterfaceItem, ntree->interface_items()[index]);
  return true;
}
    ...
    switch (item->item_type) {
      case NODE_INTERFACE_SOCKET: {
        ...
        if (STREQ(socket->name, key)) {
          *r_ptr = RNA_pointer_create_with_ancestors(*ptr, &RNA_NodeTreeInterfaceSocket, socket);
          return true;
        }
        break;
      }
      case NODE_INTERFACE_PANEL: {
        ...
        if (STREQ(panel->name, key)) {
          *r_ptr = RNA_pointer_create_with_ancestors(*ptr, &RNA_NodeTreeInterfacePanel, panel);
          return true;
        }
        break;
      }
