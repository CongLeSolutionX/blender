        ...
        for (node = static_cast<bNode *>(ntree->nodes.first); node; node = node->next) {
          if (ELEM(node->type, SH_NODE_VALTORGB, CMP_NODE_VALTORGB, TEX_NODE_VALTORGB)) {
            if (node->storage == ptr->data) {
               ...
               */
              PointerRNA node_ptr = RNA_pointer_create_isolated(id, &RNA_Node, node);
  ...
  if (ptr->owner_id) {
    ...
    switch (GS(id->name)) {
      case ID_NT: {
        ...
        for (node = static_cast<bNode *>(ntree->nodes.first); node; node = node->next) {
          if (ELEM(node->type, SH_NODE_VALTORGB, CMP_NODE_VALTORGB, TEX_NODE_VALTORGB)) {
            ramp_ptr = RNA_pointer_create_isolated(id, &RNA_ColorRamp, node->storage);
            COLRAMP_GETPATH;
          }
        }
        break;
      ...
      case ID_LS: {
        ...
        for (link = (LinkData *)listbase.first; link; link = link->next) {
          ramp_ptr = RNA_pointer_create_isolated(id, &RNA_ColorRamp, link->data);
          COLRAMP_GETPATH;
        }
