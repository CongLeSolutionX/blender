...
{
  bContext *C = (bContext *)ptr->data;
  return RNA_pointer_create_with_ancestors(*ptr, &RNA_Window, CTX_wm_window(C));
}
...
{
  bContext *C = (bContext *)ptr->data;
  PointerRNA newptr = RNA_pointer_create_isolated(
...
{
  bContext *C = (bContext *)ptr->data;
  PointerRNA newptr = RNA_pointer_create_isolated(nullptr, &RNA_GizmoGroup, CTX_wm_gizmo_group(C));
  return newptr;
}
...
{
  bContext *C = (bContext *)ptr->data;
  return RNA_pointer_create_isolated(nullptr, &RNA_AssetRepresentation, CTX_wm_asset(C));
}

static PointerRNA rna_Context_main_get(PointerRNA *ptr)
{
  bContext *C = (bContext *)ptr->data;
  return RNA_pointer_create_isolated(nullptr, &RNA_BlendData, CTX_data_main(C));
}
...
static PointerRNA rna_Context_view_layer_get(PointerRNA *ptr)
{
  ...
  return RNA_pointer_create_with_ancestors(scene_ptr, &RNA_ViewLayer, CTX_data_view_layer(C));
}
...
static PointerRNA rna_Context_tool_settings_get(PointerRNA *ptr)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  return RNA_pointer_create_with_ancestors(
      scene_ptr, &RNA_ToolSettings, CTX_data_tool_settings(C));
}

static PointerRNA rna_Context_preferences_get(PointerRNA * /*ptr*/)
{
  PointerRNA newptr = RNA_pointer_create_isolated(nullptr, &RNA_Preferences, &U);
  return newptr;
}
