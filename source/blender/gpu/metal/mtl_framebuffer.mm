...
bool MTLFrameBuffer::check(char err_out[256])
{
  ...
  for (int col_att = 0; col_att < this->get_attachment_count(); col_att++) {
    ...
    if (att.used) {
      if (att.texture->internal_gpu_image_usage_flags & GPU_TEXTURE_USAGE_ATTACHMENT) {
        if (first) {
  ...
  if (depth_att.used) {
    if (first) {
      ...
      first = false;
      valid = (depth_att.texture->internal_gpu_image_usage_flags & GPU_TEXTURE_USAGE_ATTACHMENT);
  ...
  if (stencil_att.used) {
    if (first) {
      ...
      first = false;
      valid = (stencil_att.texture->internal_gpu_image_usage_flags & GPU_TEXTURE_USAGE_ATTACHMENT);
      if (!valid) {
        const char *format =
