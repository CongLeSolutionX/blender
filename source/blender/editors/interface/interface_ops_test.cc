...
class CopyDriversToSelected : public testing::Test {
 public:
  ...
  void SetUp() override
  {
    ...
    cube_ptr = RNA_pointer_create_isolated(&cube->id, &RNA_Object, &cube->id);
    ...
    suzanne_ptr = RNA_pointer_create_isolated(&suzanne->id, &RNA_Object, &suzanne->id);
    ...
    PointerRNA cube_ptr = RNA_pointer_create_isolated(&cube->id, &RNA_Object, &cube->id);
