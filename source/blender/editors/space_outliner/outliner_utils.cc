...
bool ED_outliner_give_rna_under_cursor(bContext *C, const int mval[2], PointerRNA *r_ptr)
{
  ...
  switch (tselem->type) {
    case TSE_BONE: {
      ...
      *r_ptr = RNA_pointer_create_isolated(tselem->id, &RNA_Bone, bone);
      break;
    }
    case TSE_POSE_CHANNEL: {
      bPoseChannel *pchan = (bPoseChannel *)te->directdata;
      *r_ptr = RNA_pointer_create_isolated(tselem->id, &RNA_PoseBone, pchan);
      break;
    }
    case TSE_EBONE: {
      EditBone *bone = (EditBone *)te->directdata;
      *r_ptr = RNA_pointer_create_isolated(tselem->id, &RNA_EditBone, bone);
      break;
    }
