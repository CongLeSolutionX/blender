                                                  ...
                                                  const char *propname)
{
  ...
  PointerRNA ptr = RNA_pointer_create_isolated(id, struct_rna, data);
       ...
       ob_iter = static_cast<Object *>(ob_iter->id.next))
  {
    if (BKE_object_is_child_recursive(ob_parent, ob_iter)) {
      if (ob) {
        ...
        ptr = RNA_pointer_create_isolated(&scene->id, &RNA_ObjectBase, base_iter);
      }
                                                        ...
                                                        PointerRNA *ptr)
{
  if (collection) {
    ...
    *ptr = RNA_pointer_create_isolated(&scene->id, &RNA_LayerCollection, layer_collection);
  }
}
    ...
    Scene *scene, ViewLayer *view_layer, Collection *collection, Object *ob, PointerRNA *ptr)
{
  if (collection) {
    ...
    *ptr = RNA_pointer_create_isolated(&scene->id, &RNA_ObjectBase, base);
  }
}
                                                        ...
                                                        RestrictPropertiesActive *props_active)
{
  ...
  *collection_ptr = RNA_id_pointer_create(&collection->id);
  if (layer_collection != nullptr) {
    *layer_collection_ptr = RNA_pointer_create_isolated(
    ...
    if (te->ys + 2 * UI_UNIT_Y >= region->v2d.cur.ymin && te->ys <= region->v2d.cur.ymax) {
               ...
               (te->flag & TE_CHILD_NOT_IN_COLLECTION))
      {
        ...
        if (space_outliner->show_restrict_flags & SO_RESTRICT_HIDE) {
          ...
          if (base) {
            PointerRNA base_ptr = RNA_pointer_create_isolated(&scene->id, &RNA_ObjectBase, base);
        ...
        PointerRNA ptr = RNA_pointer_create_isolated(tselem->id, &RNA_Constraint, con);
        ...
        PointerRNA ptr = RNA_pointer_create_isolated(tselem->id, &RNA_Modifier, md);
        ...
        PointerRNA ptr = RNA_pointer_create_isolated(&arm->id, &RNA_Bone, bone);
        ...
        if (node.is_layer()) {
          ptr = RNA_pointer_create_isolated(tselem->id, &RNA_GreasePencilLayer, &node);
          hide_prop = RNA_struct_type_find_property(&RNA_GreasePencilLayer, "hide");
        }
        else if (node.is_group()) {
          ptr = RNA_pointer_create_isolated(tselem->id, &RNA_GreasePencilLayerGroup, &node);
