    ...
    LISTBASE_FOREACH (SpaceLink *, sl, &area->spacedata) {
      switch (sl->spacetype) {
        case SPACE_SEQ: {
          ...
          proxy_sizes |= IMB_Proxy_Size(SEQ_rendersize_to_proxysize(sseq->render_size));
        }
      }
    }
  }
...
static void sequencer_add_draw(bContext * /*C*/, wmOperator *op)
{
  ...
  /* Image template. */
  PointerRNA imf_ptr = RNA_pointer_create_isolated(nullptr, &RNA_ImageFormatSettings, imf);
