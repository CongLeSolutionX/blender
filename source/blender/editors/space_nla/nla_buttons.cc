  ...
  LISTBASE_FOREACH (bAnimListElem *, ale, &anim_data) {
    switch (ale->type) {
      case ANIMTYPE_NLATRACK: /* NLA Track - The primary data type which should get caught */
      {
        ...
        if (adt_ptr) {
          /* AnimData pointer */
          *adt_ptr = RNA_pointer_create_isolated(ale->id, &RNA_AnimData, adt);
        }
        if (nlt_ptr) {
          /* NLA-Track pointer */
          *nlt_ptr = RNA_pointer_create_isolated(ale->id, &RNA_NlaTrack, nlt);
        }
        if (strip_ptr) {
          /* NLA-Strip pointer */
          NlaStrip *strip = BKE_nlastrip_find_active(nlt);
          *strip_ptr = RNA_pointer_create_isolated(ale->id, &RNA_NlaStrip, strip);
        }
      ...
      case ANIMTYPE_DSVOLUME: {
        ...
        if (ale->adt && adt_ptr) {
          ...
          /* AnimData pointer */
          if (adt_ptr) {
            *adt_ptr = RNA_pointer_create_isolated(id, &RNA_AnimData, ale->adt);
          }
