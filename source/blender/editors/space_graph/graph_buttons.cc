...
static void graph_panel_cursor_header(const bContext *C, Panel *panel)
{
  ...
  PointerRNA spaceptr = RNA_pointer_create_isolated(&screen->id, &RNA_SpaceGraphEditor, sipo);
...
static void graph_panel_properties(const bContext *C, Panel *panel)
{
  ...
  /* F-Curve pointer */
  PointerRNA fcu_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_FCurve, fcu);
...
static void graph_panel_key_properties(const bContext *C, Panel *panel)
{
  ...
  if (get_active_fcurve_keyframe_edit(fcu, &bezt, &prevbezt)) {
    ...
    PointerRNA bezt_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_Keyframe, bezt);
...
static void graph_panel_driverVar__singleProp(uiLayout *layout, ID *id, DriverVar *dvar)
{
  ...
  PointerRNA dtar_ptr = RNA_pointer_create_isolated(id, &RNA_DriverTarget, dtar);
...
static void graph_panel_driverVar__rotDiff(uiLayout *layout, ID *id, DriverVar *dvar)
{
  ...
  /* initialize RNA pointer to the target */
  PointerRNA dtar_ptr = RNA_pointer_create_isolated(id, &RNA_DriverTarget, dtar);
  PointerRNA dtar2_ptr = RNA_pointer_create_isolated(id, &RNA_DriverTarget, dtar2);
  ...
  if (dtar->id && GS(dtar->id->name) == ID_OB && ob1->pose) {
    PointerRNA tar_ptr = RNA_pointer_create_isolated(dtar->id, &RNA_Pose, ob1->pose);
  ...
  if (dtar2->id && GS(dtar2->id->name) == ID_OB && ob2->pose) {
    PointerRNA tar_ptr = RNA_pointer_create_isolated(dtar2->id, &RNA_Pose, ob2->pose);
...
static void graph_panel_driverVar__transChan(uiLayout *layout, ID *id, DriverVar *dvar)
{
  ...
  if (dtar->id && GS(dtar->id->name) == ID_OB && ob->pose) {
    PointerRNA tar_ptr = RNA_pointer_create_isolated(dtar->id, &RNA_Pose, ob->pose);
    uiItemPointerR(
        col, &dtar_ptr, "bone_target", &tar_ptr, "bones", IFACE_("Bone"), ICON_BONE_DATA);
  }
...
static void graph_draw_driven_property_enabled_btn(uiLayout *layout,
                                                   ...
                                                   const char *label)
{
  PointerRNA fcurve_ptr = RNA_pointer_create_isolated(id, &RNA_FCurve, fcu);
                                             ...
                                             const bool is_popover)
{
  ...
  PointerRNA driver_ptr = RNA_pointer_create_isolated(id, &RNA_Driver, driver);
  ...
  LISTBASE_FOREACH (DriverVar *, dvar, &driver->variables) {
    ...
    /* 1) header panel */
    box = uiLayoutBox(col);
    PointerRNA dvar_ptr = RNA_pointer_create_isolated(id, &RNA_DriverVariable, dvar);
...
static void graph_panel_drivers_popover(const bContext *C, Panel *panel)
{
  ...
  if (but) {
    ...
    if (fcu && fcu->driver) {
      ...
      PointerRNA ptr_fcurve = RNA_pointer_create_isolated(id, &RNA_FCurve, fcu);
