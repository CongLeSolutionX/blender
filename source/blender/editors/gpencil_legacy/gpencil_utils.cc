  ...
  if (area) {
    ...
    switch (area->spacetype) {
      ...
      case SPACE_SEQ: /* Sequencer */
      {
        ...
        if (r_ptr) {
          *r_ptr = RNA_pointer_create_isolated(screen_id, &RNA_SpaceSequenceEditor, sseq);
        }
        return &sseq->gpd;
      }
      case SPACE_IMAGE: /* Image/UV Editor */
      {
        ...
        /* For now, Grease Pencil data is associated with the space... */
        if (r_ptr) {
          *r_ptr = RNA_pointer_create_isolated(screen_id, &RNA_SpaceImageEditor, sima);
        }
        return &sima->gpd;
      }
      case SPACE_CLIP: /* Nodes Editor */
      {
        ...
        if (clip) {
          if (sc->gpencil_src == SC_GPENCIL_SRC_TRACK) {
            ...
            if (r_ptr) {
              *r_ptr = RNA_pointer_create_isolated(&clip->id, &RNA_MovieTrackingTrack, track);
            }
            return &track->gpd;
          }
          if (r_ptr) {
