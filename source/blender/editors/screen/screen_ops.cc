...
{
  PointerRNA ptr = RNA_pointer_create_isolated(nullptr, &RNA_PreferencesView, &U);
  uiItemR(
...
static int userpref_show_exec(bContext *C, wmOperator *op)
{
  ...
  if (prop && RNA_property_is_set(op->ptr, prop)) {
    ...
    PointerRNA pref_ptr = RNA_pointer_create_isolated(nullptr, &RNA_Preferences, &U);
...
static int space_type_set_or_cycle_exec(bContext *C, wmOperator *op)
{
  ...
  ScrArea *area = CTX_wm_area(C);
  PointerRNA ptr = RNA_pointer_create_isolated((ID *)CTX_wm_screen(C), &RNA_Area, area);
                                   ...
                                   PropertyRNA **r_prop)
{
  ...
  switch (area->spacetype) {
    case SPACE_PROPERTIES:
      *r_ptr = RNA_pointer_create_isolated(
          &screen->id, &RNA_SpaceProperties, area->spacedata.first);
      propname = "context";
      break;
    case SPACE_USERPREF:
      *r_ptr = RNA_pointer_create_isolated(nullptr, &RNA_Preferences, &U);
