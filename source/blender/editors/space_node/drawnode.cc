...
static void node_buts_value(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  PointerRNA sockptr = RNA_pointer_create_isolated(ptr->owner_id, &RNA_NodeSocket, output);
...
static void node_composit_buts_image(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  PointerRNA iuserptr = RNA_pointer_create_isolated(ptr->owner_id, &RNA_ImageUser, node->storage);
  uiLayoutSetContextPointer(layout, "image_user", &iuserptr);
  uiTemplateID(layout, C, ptr, "image", "IMAGE_OT_new", "IMAGE_OT_open", nullptr);
  if (!node->id) {
    return;
...
static void node_composit_buts_cryptomatte(uiLayout *layout, bContext *C, PointerRNA *ptr)
{
  ...
  if (node->custom1 == CMP_NODE_CRYPTOMATTE_SOURCE_RENDER) {
    ...
    PointerRNA imaptr = RNA_pointer_get(ptr, "image");
    PointerRNA iuserptr = RNA_pointer_create_isolated(
...
static void node_texture_buts_proc(uiLayout *layout, bContext * /*C*/, PointerRNA *ptr)
{
  ...
  PointerRNA tex_ptr = RNA_pointer_create_isolated(id, &RNA_Texture, tex);
                                         ...
                                         PointerRNA *node_ptr)
{
  ...
  if (imtype == R_IMF_IMTYPE_MULTILAYER) {
    ...
    inputptr = RNA_pointer_create_isolated(&ntree->id, &RNA_NodeOutputFileSlotLayer, input);
    ...
    NodeImageMultiFileSocket *input = (NodeImageMultiFileSocket *)sock->storage;
    uiBlock *block;
    inputptr = RNA_pointer_create_isolated(&ntree->id, &RNA_NodeOutputFileSlotFile, input);
...
static void std_node_socket_interface_draw(ID *id,
                                           ...
                                           uiLayout *layout)
{
  PointerRNA ptr = RNA_pointer_create_isolated(id, &RNA_NodeTreeInterfaceSocket, interface_socket);
                                                   ...
                                                   const bool selected)
{
      ...
      snode.overlay.flag & SN_OVERLAY_SHOW_WIRE_COLORS)
  {
    ...
    PointerRNA from_node_ptr = RNA_pointer_create_isolated(
        &const_cast<ID &>(node_tree.id), &RNA_Node, link.fromnode);
    PointerRNA to_node_ptr = RNA_pointer_create_isolated(
