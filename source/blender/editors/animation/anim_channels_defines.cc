...
{
  *r_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_ActionGroup, ale->data);
  *r_prop = RNA_struct_name_property(r_ptr->type);
...
static bool acf_fcurve_name_prop(bAnimListElem *ale, PointerRNA *r_ptr, PropertyRNA **r_prop)
{
  ...
  if (fcu->flag & FCURVE_DISABLED) {
    *r_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_FCurve, ale->data);
...
static bool acf_action_slot_name_prop(bAnimListElem *ale, PointerRNA *r_ptr, PropertyRNA **r_prop)
{
  ...
  *r_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_ActionSlot, slot);
...
static bool acf_shapekey_name_prop(bAnimListElem *ale, PointerRNA *r_ptr, PropertyRNA **r_prop)
{
  ...
  /* if the KeyBlock had a name, use it, otherwise use the index */
  if (kb && kb->name[0]) {
    *r_ptr = RNA_pointer_create_isolated(ale->id, &RNA_ShapeKey, kb);
    *r_prop = RNA_struct_name_property(r_ptr->type);
...
{
  if (ale->data) {
    *r_ptr = RNA_pointer_create_isolated(ale->id, &RNA_GPencilLayer, ale->data);
    *r_prop = RNA_struct_name_property(r_ptr->type);
...
static bool layer_name_prop(bAnimListElem *ale, PointerRNA *r_ptr, PropertyRNA **r_prop)
{
  ...
  *r_ptr = RNA_pointer_create_isolated(ale->id, &RNA_GreasePencilLayer, ale->data);
  *r_prop = RNA_struct_name_property(r_ptr->type);
...
{
  if (ale->data) {
    *r_ptr = RNA_pointer_create_isolated(ale->id, &RNA_MaskLayer, ale->data);
    *r_prop = RNA_struct_name_property(r_ptr->type);
...
{
  if (ale->data) {
    *r_ptr = RNA_pointer_create_isolated(ale->id, &RNA_NlaTrack, ale->data);
    *r_prop = RNA_struct_name_property(r_ptr->type);
...
{
  if (ale->data) {
    *r_ptr = RNA_pointer_create_isolated(ale->fcurve_owner_id, &RNA_Action, ale->data);
    *r_prop = RNA_struct_name_property(r_ptr->type);
                                             ...
                                             const int array_index)
{
  ...
  PointerRNA ptr = RNA_pointer_create_isolated(ale->id, &RNA_GreasePencilLayer, ale->data);
  PointerRNA id_ptr = RNA_id_pointer_create(ale->id);
        ...
        ale->type == ANIMTYPE_SHAPEKEY)
    {
      ...
      if (ale->owner) { /* Slider using custom RNA Access ---------- */
        if (ale->type == ANIMTYPE_NLACURVE) {
          ...
          /* create RNA pointers */
          PointerRNA ptr = RNA_pointer_create_isolated(ale->id, &RNA_NlaStrip, strip);
        ...
        /* Special for Grease Pencil Layer. */
          ...
          if ((gpd != nullptr) && ((gpd->flag & GP_DATA_ANNOTATIONS) == 0)) {
            ...
            /* Create the RNA pointers. */
            ptr = RNA_pointer_create_isolated(ale->id, &RNA_GPencilLayer, ale->data);
            PointerRNA id_ptr = RNA_id_pointer_create(ale->id);
            int icon;
