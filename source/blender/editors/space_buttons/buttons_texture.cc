  ...
  if (ntree) {
    LISTBASE_FOREACH (bNode *, node, &ntree->nodes) {
      if (node->type == CMP_NODE_TEXTURE) {
        PointerRNA ptr = RNA_pointer_create_isolated(&ntree->id, &RNA_Node, node);
    ...
    blender::Set<const bNodeTree *> &handled_groups)
{
  ...
  for (bNode *node : node_tree->all_nodes()) {
    ...
    LISTBASE_FOREACH (bNodeSocket *, socket, &node->inputs) {
      ...
      PointerRNA ptr = RNA_pointer_create_isolated(&node_tree->id, &RNA_NodeSocket, socket);
                                               ...
                                               SpaceProperties *sbuts)
{
  ...
  if (ob) {
    ...
    if (ob->pd && ob->pd->forcefield == PFIELD_TEXTURE) {
      ...
      PointerRNA ptr = RNA_pointer_create_isolated(&ob->id, &RNA_FieldSettings, ob->pd);
      prop = RNA_struct_find_property(&ptr, "texture");
  ...
  if (brush) {
    ...
    /* texture */
    PointerRNA ptr = RNA_pointer_create_isolated(&brush->id, &RNA_BrushTextureSlot, &brush->mtex);
    prop = RNA_struct_find_property(&ptr, "texture");
    ...
    /* mask texture */
    ptr = RNA_pointer_create_isolated(&brush->id, &RNA_BrushTextureSlot, &brush->mask_mtex);
    prop = RNA_struct_find_property(&ptr, "texture");
