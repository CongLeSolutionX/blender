...
static void extract_face_corner_dot_normals_mesh(const MeshRenderData &mr, MutableSpan<GPUType> normals)
{
...
template<typename GPUType>
void extract_face_corner_dot_normals_bm(const MeshRenderData &mr, MutableSpan<GPUType> normals)
{
  const GPUType invalid_normal = convert_normal<GPUType>(float3(0));
  threading::parallel_for(IndexRange(mr.faces_num), 4096, [&](const IndexRange range) {
    for (const int i : range) {
...
void extract_face_corner_dot_normals(const MeshRenderData &mr, const bool use_hq, gpu::VertBuf &vbo)
{
  if (use_hq) {
    ...
    if (mr.extract_type == MeshExtractType::Mesh) {
      extract_face_corner_dot_normals_mesh(mr, vbo_data);
    }
    else {
      extract_face_corner_dot_normals_bm(mr, vbo_data);
    }
  }
  else {
    static GPUVertFormat format = {0};
