...
static const GPUVertFormat &get_cdots_pos_format()
{
...
static const GPUVertFormat &get_cdots_nor_format_subdiv()
{
  static GPUVertFormat format = {0};
  if (format.attr_len == 0) {
...
// TODO: do the corner dots calculation for mesh
static void extract_face_corner_dot_positions_mesh(const MeshRenderData &mr, MutableSpan<float3> vbo_data)
{
...
// TODO: do the corner dots calculation for bmesh
static void extract_face_corner_dot_positions_bm(const MeshRenderData &mr, MutableSpan<float3> vbo_data)
{
...
{
  GPU_vertbuf_init_with_format(vbo, get_cdots_pos_format());
  GPU_vertbuf_data_alloc(vbo, mr.corners_num + mr.loose_indices_num + 1);

  MutableSpan vbo_data = vbo.data<float3>();
  if (mr.extract_type == MeshExtractType::Mesh) {
    extract_face_corner_dot_positions_mesh(mr, vbo_data);
  }
  else {
    extract_face_corner_dot_positions_bm(mr, vbo_data);
  }
}
                              ...
                              gpu::IndexBuf &fdots)
{
  ...
  if (fdots_nor) {
    GPU_vertbuf_init_build_on_device(
        *fdots_nor, get_cdots_nor_format_subdiv(), subdiv_cache.num_coarse_faces);
  }
  GPU_vertbuf_init_build_on_device(
      fdots_pos, get_cdots_pos_format(), subdiv_cache.num_coarse_faces);
  GPU_indexbuf_init_build_on_device(&fdots, subdiv_cache.num_coarse_faces);
  draw_subdiv_build_cdots_buffers(subdiv_cache, &fdots_pos, fdots_nor, &fdots);
}
