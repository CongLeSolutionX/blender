      ...
      case PROP_POINTER: {
        ...
        if (raise_error) {
          if (pyrna_struct_validity_check(param) == -1) {
            ...
            PointerRNA tmp = RNA_pointer_create_isolated(nullptr, ptr_type, nullptr);
            PyErr_Format(PyExc_TypeError,
                         "%.200s %.200s.%.200s expected a %.200s type, not %.200s",
                         error_prefix,
                         RNA_struct_identifier(ptr->type),
...
static PyObject *pyrna_struct_bl_rna_get_subclass(PyObject *cls, PyObject *args)
{
  ...
  if (srna_base == &RNA_Node) {
    ...
    if (nt) {
      PointerRNA ptr = RNA_pointer_create_isolated(nullptr, &RNA_Struct, nt->rna_ext.srna);
      return pyrna_struct_CreatePyObject(&ptr);
    }
  }
  else {
...
static void pyrna_dir_members_rna(PyObject *list, PointerRNA *ptr)
{
  ...
  {
    PointerRNA tptr = RNA_pointer_create_isolated(nullptr, &RNA_Struct, ptr->type);
...
#  if 0
  if ((ret == nullptr) /* || BPy_PropDeferred_CheckTypeExact(ret) */) {
    ...
    if (srna) {
      ...
      if (prop) {
        PointerRNA tptr = RNA_pointer_create_isolated(nullptr, &RNA_Property, prop);
...
{
  PointerRNA tptr = RNA_pointer_create_isolated(nullptr, &RNA_Property, self->prop);
      ...
      case PROP_POINTER: {
        ...
        if (flag_parameter & PARM_RNAPTR) {
          ...
          if (RNA_struct_is_ID(ptype)) {
            ...
            newptr = RNA_pointer_create_isolated(ptr->owner_id, ptype, *(void **)data);
          }
          newptr_p = &newptr;
        }
...
/* For testing. */
  ...
  PointerRNA funcptr = RNA_pointer_create_isolated(self_ptr->owner_id, &RNA_Function, self_func);
...
static void pyrna_subtype_set_rna(PyObject *newclass, StructRNA *srna)
{
  ...
  PointerRNA ptr = RNA_pointer_create_isolated(nullptr, &RNA_Struct, srna);
...
#  endif
  ...
  if (err != -1 && (is_staticmethod || is_classmethod || py_class_instance)) {
    ...
    if (item_type_valid) {
      funcptr = RNA_pointer_create_isolated(nullptr, &RNA_Function, func);
                                                 ...
                                                 const char **r_prop_identifier)
{
  ...
  LISTBASE_FOREACH (LinkData *, link, lb) {
    ...
    if (RNA_property_type(prop) == PROP_POINTER && !RNA_property_builtin(prop)) {
      PointerRNA tptr = RNA_pointer_create_isolated(nullptr, &RNA_Struct, srna_props);
