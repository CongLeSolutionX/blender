...
void BKE_callback_exec_id_depsgraph(Main *bmain, ID *id, Depsgraph *depsgraph, eCbEvent evt)
{
  ...
  PointerRNA depsgraph_ptr = RNA_pointer_create_isolated(nullptr, &RNA_Depsgraph, depsgraph);
...
void BKE_callback_exec_string(Main *bmain, eCbEvent evt, const char *str)
{
  ...
  data.value = str;
  PointerRNA str_ptr = RNA_pointer_create_isolated(nullptr, &RNA_PrimitiveString, &data);
