                   ...
                   MutableSpan<float> dst_knots)
{
  ...
  Array<float> knots(curves::nurbs::knots_num(points_num, order, is_cyclic));
  calculate_knots(points_num, mode, order, is_cyclic, knots);
  compact_knots(order, knots, dst_knots);
}
