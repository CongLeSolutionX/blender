...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  auto &total_layers = b.add_output<decl::Int>("Layer Count")
                           ...
                           .available(false);
  auto &total_bodies = b.add_output<decl::Int>("Body Count")
                           .make_available([](bNode &node) {
                             node.custom1 = int16_t(GeometryComponent::Type::Physics);
                           })
                           .available(false);
  auto &total_constraints = b.add_output<decl::Int>("Constraint Count")
                                .make_available([](bNode &node) {
                                  node.custom1 = int16_t(GeometryComponent::Type::Physics);
                                })
                                .available(false);
  auto &total_shapes = b.add_output<decl::Int>("Shape Count")
                           .make_available([](bNode &node) {
                             node.custom1 = int16_t(GeometryComponent::Type::Physics);
                           })
                           .available(false);
  ...
  switch (component) {
    ...
    case GeometryComponent::Type::Physics: {
      if (const PhysicsComponent *component = geometry_set.get_component<PhysicsComponent>()) {
        const AttributeAccessor attributes = *component->attributes();
        params.set_output("Body Count", attributes.domain_size(AttrDomain::Point));
        params.set_output("Constraint Count", attributes.domain_size(AttrDomain::Edge));
        params.set_output("Shape Count", attributes.domain_size(AttrDomain::Instance));
      }
      else {
        params.set_default_remaining_outputs();
      }
